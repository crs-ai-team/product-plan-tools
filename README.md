...
開啟時預設帶下個月，因為排班多半是提前規劃；12 月時自動跨到隔年 1 月。月份不會被記成偏好，每次開啟都回到下個月。

抓取台灣行事曆，只在實際要上班的日子排工時，國定假日、彈性放假與補行上班日都會正確反映。表格是完整的一個月，假日欄以「休」標示且不計入合計。

行事曆來源為 [ruyut/TaiwanCalendar](https://github.com/ruyut/TaiwanCalendar)，透過 jsDelivr 取得，每年的資料在瀏覽器快取七天。取不到資料時會自動退回「週一至週五視為工作日」的推算，並在面板上標示，因此離線或 CDN 異常時仍可使用。目前上游提供 2017 到 2027 年。

### 快速產生

自訂天數，不對應實際日期，適合只想看幾種分配樣式的情況。

## 功能

- **精確均勻抽樣**：以動態規劃計算合法組合數後逐格抽樣，直接命中符合條件的結果，不需要反覆重抽。
- **可重現**：每張表都對應一組 8 碼種子。貼上別人的種子，或直接開分享連結，就能看到完全相同的表格；連結也會記住當時所在的分頁。
- **即時重算**：任何參數一改動，表格、統計數字與分享連結同步更新，不會出現彼此對不上的狀態。
- **色階表格**：底色深淺對應投入時數，效果等同試算表的色階格式化；表頭、專案欄與合計欄固定，整月 31 欄橫向捲動時也好讀。
- **可改名**：點專案名稱即可改成實際的專案名，兩種模式共用同一份，會保存在瀏覽器裡。
- **一鍵複製**：把數字區塊複製成 tab 分隔的文字，直接貼進既有的試算表。不含表頭與專案名稱，欄數固定為當月天數，工作日一律給數字（沒排到就是 0），假日欄留空，貼上後位置完全對齊且不會把 0 塞進假日格。
...
| --- | --- | --- |
| 專案數量 | 表格的列數 | 1–20 |
| 年 / 月 | 整月模式要排的月份，預設為下個月 | 2017/01–2027/12 |
| 天數 | 快速模式的表格欄數 | 1–31 |
...
整月模式先向行事曆問出這個月有幾個工作日，再把工作日當成天數丟給產生器，最後把結果攤回整個月份，假日一律補 0。
...
「複製表格」產生的內容規則如下，設計目標是貼進以整月為欄位、假日欄留白的既有表單：
...
每一列的欄數固定等於當月天數，因此不會因為假日而錯位。要含表頭與專案名稱的完整表格請改用 CSV 或 Excel 下載。
...
├── components/
│   ├── MonthPanel.tsx    # 整月工作日模式
│   ├── QuickPanel.tsx    # 快速產生模式
│   ├── MonthControls.tsx # 月份參數面板
│   ├── ControlPanel.tsx  # 快速模式參數面板
│   ├── HoursFields.tsx   # 兩種模式共用的工時設定
│   ├── SeedInput.tsx     # 種子輸入
│   ├── NumberField.tsx   # 滑桿 + 數字輸入
│   ├── PanelActions.tsx  # 錯誤提示與操作按鈕
│   ├── ResultCard.tsx    # 統計數字與匯出按鈕
│   ├── ResultTable.tsx   # 熱力表格
│   └── ThemeToggle.tsx   # 深淺色切換
├── hooks/
│   ├── useProjectNames.ts # 專案名稱，兩種模式共用
│   └── useToast.ts        # 短暫提示訊息
└── lib/
    ├── calendar.ts       # 台灣行事曆抓取、快取與備援
